...
function combine(arrayYo, arrayEr) {
    var combinedArray = [];

    for (var i = 0; i < arrayYi.length; i++) {
        combinedArray.push(arrayYi[i]);
    }
    for (var i = 0; i < arrayEr.length; i++) {
        combinedArray.push(arrayEr[i]);
    }
...
function subtractArray(diYiArray, diErArray) {
    var resulting = [];

    for(var i = 0; i < diYiArray.length; i++) {
        if(!diErArray.includes(diYiArray[i])) {
            resulting.push(diYiArray[i]);
        }
    }
